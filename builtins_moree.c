...
/**
 * builtin_help - Displays the environment in which the shell is running
 * @data: struct for the program's data
 * Return: zero if sucess, or other number if its declared in the arguments
 */
int builtin_help(data_of_program *data)
{
	...
	for (z = 0; mensajes[z]; z++)
	{
		length = str_length(data->tokens[1]);
		if (str_compare(data->tokens[1], mensajes[z], length))
		{
			_print(mensajes[z] + length + 1);
			return (1);
		}
	}
...
int builtin_alias(data_of_program *data)
{
	...
	{/* if there are arguments, set or print each env variable*/
		if (count_characters(data->tokens[z], "="))
			set_alias(data->tokens[z], data);
		else
			print_alias(data, data->tokens[z]);
	}
