...
char *env_get_key(char *key, data_of_program *data)
{
	...
	for (p = 0; data->env[p]; p++)
	{/* Iterates through the environ and check for coincidence of the vame */
		if (str_compare(key, data->env[p], key_length) &&
		 data->env[p][key_length] == '=')
		{/* returns the value of the key NAME=  when find it*/
			return (data->env[p] + key_length + 1);
		}
	}
...
{
	int w, key_length = 0, is_new_key = 1;
	...
	for (w = 0; data->env[w]; w++)
	{/* Iterates through the environ and check for coincidence of the vame */
		if (str_compare(key, data->env[w], key_length) &&
		 data->env[w][key_length] == '=')
	...
	data->env[w] = str_concat(str_duplicate(key), "=");
	data->env[w] = str_concat(data->env[w], value);
